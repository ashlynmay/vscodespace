        ...
        if not request.form.get("symbol") or not request.form.get("shares"):
            ...
            if cash - total > 0:
                ...
                if existing_total:
                    ...
                    db.execute("UPDATE users SET cash = ?", (cash - updated_total))
                else:
...
def sell():
    ...
    if request.method == "POST":
        ...
        stock = stock.upper()
        if not  or not request.form.get("shares"):
            return apology("must provide stock symbol and the amount of shares", 402)
        elif lookup(request.form.get("symbol")) == None:
            return apology("stock not found", 402)
        elif int(request.form.get("shares")) < 1:
        ...
        total_result = db.execute("SELECT total FROM purchases WHERE id = ? AND stock = ?", session["user_id"], request.form.get("symbol"))
        ...
        if int(request.form.get("shares")) > shares:
            ...
            db.execute("INSERT INTO history (stock, shares, price, total, type, dt, id) VALUES (?, ?, ?, ?, 'SELL', ?, ?)", request.form.get("symbol"), request.form.get("shares"), price, ntotal, current_datetime, session["user_id"])
            return redirect("/")
