...
def sell():
    ...
    if request.method == "POST":
        ...
        if int(request.form.get("shares")) > shares:
            ...
            db.execute("INSERT INTO history (stock, shares, price, total, type, dt, id) VALUES (?, ?, ?, ?, 'sell', ?)", request.form.get("symbol"), request.form.get("shares"), price, ntotal, current_datetime)
            return redirect("/")
