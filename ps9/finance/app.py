...
def sell():
    ...
    if request.method == "POST":
        ...
        updated_total = total - (request.form.get("shares") * price)
        if request.form.get("shares") > shares:
            ...
            db.execute("INSERT into history (stock, shares, price, total, type, dt) VALUES (?, ?, ?, ?, 'sell', ?)", request.form.get("symbol"), request.form.get("shares"), request.form.get("symbol"))