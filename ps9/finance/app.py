...
def index():
    ...
    purchased = db.execute("SELECT total FROM purchases WHERE id = ?", session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        if not request.form.get("symbol") or not request.form.get("shares"):
        ...
        elif (request.form.get("shares").isdecimal()) == False:
            ...
            newcash = float(cash) - float(total)
            if cash - total > 0:
                current_datetime = datetime.now()
                if not db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='purchases'"):
                    db.execute(
                        "CREATE TABLE purchases (id INTEGER NOT NULL, stock TEXT NOT NULL, shares INTEGER NOT NULL, price FLOAT NOT NULL, total FLOAT NOT NULL)")
                if not db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='history'"):
                    db.execute(
                        "CREATE TABLE history (id INTEGER NOT NULL, stock TEXT NOT NULL, shares INTEGER NOT NULL, price FLOAT NOT NULL, total FLOAT NOT NULL, type TEXT NOT NULL, dt DATETIME)")

                existing_shares = db.execute(
                    "SELECT shares FROM purchases WHERE stock = ? AND id = ?", stock, session["user_id"])
                existing_total = db.execute(
                    "SELECT total FROM purchases WHERE stock = ? AND id = ?", stock, session["user_id"])
                ...
                if existing_total:
                    ...
                    db.execute("UPDATE users SET cash = ? WHERE id = ?",
                               float(updated_cash), session["user_id"])
                else:
    ...
    if request.method == "POST":
        ...
        # Ensure username does not exist and create a new account.
            ...
            db.execute("INSERT INTO users (username, hash) VALUES (?, ?)", 
                       request.form.get("username"), hpswd)
...
def sell():
    ...
    if request.method == "POST":
        ...
        share_result = db.execute(
            "SELECT shares FROM purchases WHERE id = ? AND stock = ?", session["user_id"], stock)
        if share_result:
        ...
        stocks = db.execute("SELECT stock")
